...
pub const SessionInfo = struct {
    /// Full info payload:
    /// name \t Attached|Detached \t idle_ms \t out_idle_ms \t waiting \t title.
    text: []u8,
    attached: bool,
    idle_ms: i64,
    /// Time since the window last produced output; drives wait --idle.
    out_idle_ms: i64,
    /// The window rang the bell and has had no input since: a generic
    /// "waiting for input" signal the ui marks. Defaults false against
    /// an older daemon whose info reply predates the field.
    waiting: bool,
...
pub fn sessionInfo(alloc: std.mem.Allocator, dir: []const u8, name: []const u8) !?SessionInfo {
    ...
    const out_idle_ms = std.fmt.parseInt(i64, it.next() orelse return error.BadResponse, 10) catch
        return error.BadResponse;
    // The remainder is either `waiting \t title` (current daemon) or
    // just `title` (a daemon predating the field). The title is
    // tab-free, so a tab in the remainder unambiguously separates an
    // optional waiting flag from the title; without one it is all title.
    const rest = it.rest();
    var waiting = false;
    var title = rest;
    if (std.mem.indexOfScalar(u8, rest, '\t')) |tab| {
        waiting = std.mem.eql(u8, rest[0..tab], "1");
        title = rest[tab + 1 ..];
    }
    return .{
        .text = result.text,
...
fn cmdLs(alloc: std.mem.Allocator, args: []const [:0]const u8) !void {
    ...
    if (json) {
        ...
        for (infos.items, 0..) |entry, i| {
            ...
            const tail = try std.fmt.allocPrint(alloc, ",\"attached\":{},\"idle_ms\":{d},\"waiting\":{},\"title\":", .{
                entry.info.attached,
                entry.info.idle_ms,
                entry.info.waiting,
            });
